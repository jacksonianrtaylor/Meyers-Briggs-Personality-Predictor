...
def main():
    ...
    for item in pairs:  
        ...
        # res1 = differential_evolution(func = test_features, bounds =  [(30, 60)], args =  ("dec_tree_model", copy.deepcopy(X), copy.deepcopy(y)),
        #                         maxiter = 10, seed = 5, updating = "deferred", workers = -1)
        # acc_list.append(-res1.fun)
        # nof_features_list.append(int(res1.x[0]))
        res2 = differential_evolution(func = test_features, bounds =  [(30, 60)], args =  ("log_reg_model", copy.deepcopy(X), copy.deepcopy(y)),
                                maxiter = 10, seed = 5, updating = "deferred", workers = -1)
        acc_list.append(-res2.fun)
        nof_features_list.append(int(res2.x[0])) 
        print("log_reg_model done for",item)   
        res3 = differential_evolution(func = test_features, bounds =  [(30, 60)], args =  ("rand_forest_model", copy.deepcopy(X), copy.deepcopy(y)),
                                maxiter = 10, seed = 5, updating = "deferred", workers = -1)
        acc_list.append(-res3.fun)
        nof_features_list.append(int(res3.x[0]))  
        print("rand_forest_model done for",item)       
        res4 = differential_evolution(func = test_features, bounds =  [(30, 60)], args =  ("naive_bays_model", copy.deepcopy(X), copy.deepcopy(y)),
                                maxiter = 10, seed = 5, updating = "deferred", workers = -1)
        acc_list.append(-res4.fun)
        nof_features_list.append(int(res4.x[0]))  
        print("naive_bays_model done for",item)  
        #Full compute time: 9.11243133544922 Minutes, bounds =  [(20, 27)], 6 fold, maxiter = 8
        #Full compute time: 9.57975435256958 Minutes, bounds = [(35, 50)], 6 fold, maxiter = 8

        #Full compute time: 9.431861571470897 Minutes, bounds =  [(30, 60)], 6 fold, maxiter = 10, (log, rand, niave)

        print(acc_list)
        print(nof_features_list)
